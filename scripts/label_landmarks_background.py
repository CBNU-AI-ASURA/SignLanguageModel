...
def process_csv_file(csv_file):
    ...
    try:
        ...
        labeled_frames = []
        background_frames = keypoint_df.copy()
        ...
        for segment in segments:
            ...
            # Background에서 해당 구간 제거
            background_frames = background_frames[~((background_frames['time'] >= start_time) & (background_frames['time'] <= end_time))]

        # Background 라벨링
        if not background_frames.empty:
            background_frames['label'] = 'background'
            labeled_frames.append(background_frames)
